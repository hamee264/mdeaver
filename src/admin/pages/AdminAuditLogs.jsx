export default function AdminAuditLogs() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div>
        <h1 style={{ fontSize: '22px', fontWeight: 800, margin: 0, letterSpacing: '-0.5px' }}>Security Audit Trail</h1>
        ...
        {[
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <span style={{ fontSize: '11px', color: '#8ea895' }}>{log.time}</span>
            </div>
            <div style={{ fontSize: '12px', color: '#d1e3d5' }}>{log.details}</div>
            <div style={{ fontSize: '10px', color: '#8ea895' }}>User: {log.user}</div>
          </div>
        ))}
      </div>
    </div>
